            ...
            while True:
                ...
                if __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/journal/1' + ('/test' if (__debugEDDN == True) else ''):
                    ...
                    if __authorised == True and __excluded == False:
                        ...
                        if __event == 'FSDJump':
                            echoLog('        - System Name: ' + __json['message']['StarSystem'])
                            if 'SystemAddress' in __json['message']:
                                echoLog('        - System Address: ' + str(__json['message']['SystemAddress']))
                            if 'StarPos' in __json['message']:
                                echoLog('        - Star Position: ' + str(__json['message']['StarPos']))
                            ...
                            if 'BodyType' in __json['message']:
                                echoLog('        - Body Type: ' + __json['message']['BodyType'])
                            if 'PlanetClass' in __json['message']:
                                echoLog('        - Planet Class: ' + __json['message']['PlanetClass'])
                            ...
                            if 'Docked' in __json['message'] and __json['message']['Docked']:
                                if 'StationName' in __json['message']:
                                    echoLog('        - Docked at Station: ' + __json['message']['StationName'])
                                if 'StationType' in __json['message']:
                                    echoLog('        - Station Type: ' + __json['message']['StationType'])
                            # Faction information
                            if 'SystemFaction' in __json['message']:
                                faction = __json['message']['SystemFaction']
                                if 'Name' in faction:
                                    echoLog('        - Controlling Faction: ' + faction['Name'])
                                if 'FactionState' in faction:
                                    echoLog('        - Faction State: ' + faction['FactionState'])
                            if 'Factions' in __json['message']:
                                echoLog('        - System Factions (' + str(len(__json['message']['Factions'])) + '):')
                                for faction in __json['message']['Factions'][:3]:  # Limit to first 3 factions
                                    if 'Name' in faction:
                                        influence = faction.get('Influence', 'Unknown')
                                        state = faction.get('FactionState', 'Unknown')
                                        echoLog('          - ' + faction['Name'] + ' (Influence: ' + str(influence) + ', State: ' + state + ')')
                            ...
                            if 'MarketID' in __json['message']:
                                echoLog('        - Market ID: ' + str(__json['message']['MarketID']))
                            # Station services
                            if 'StationServices' in __json['message']:
                                services = ', '.join(__json['message']['StationServices'][:5])  # Limit to first 5
                                echoLog('        - Services: ' + services)
                            # Station faction
                            if 'StationFaction' in __json['message']:
                                faction = __json['message']['StationFaction']
                                if 'Name' in faction:
                                    echoLog('        - Station Faction: ' + faction['Name'])
                                    
                        elif __event in ['FactionKillBond', 'Bounty']:
                            if 'StarSystem' in __json['message']:
                                echoLog('        - System Name: ' + __json['message']['StarSystem'])
                            if 'VictimFaction' in __json['message']:
                                echoLog('        - Victim Faction: ' + __json['message']['VictimFaction'])
                            if 'AwardingFaction' in __json['message']:
                                echoLog('        - Awarding Faction: ' + __json['message']['AwardingFaction'])
                ...
                # Handle NavRoute events
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/navroute/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised == True and __excluded == False:
                        ...
                        if 'Route' in __json['message']:
                            echoLog('    - Route with ' + str(len(__json['message']['Route'])) + ' systems:')
                            for i, system in enumerate(__json['message']['Route']):
                                echoLog('        ' + str(i+1) + '. ' + system['StarSystem'])
                                if 'SystemAddress' in system:
                                    echoLog('           Address: ' + str(system['SystemAddress']))
                ...
                # Handle FSS Discovery Scan events
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/fssdiscoveryscan/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        ...
                        if 'BodyCount' in __json['message']:
                            echoLog('    - Body Count: ' + str(__json['message']['BodyCount']))
                        if 'NonBodyCount' in __json['message']:
                            echoLog('    - Non-Body Count: ' + str(__json['message']['NonBodyCount']))

                    echoLog('')

                # Handle FSS Signal Discovered events  
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/fsssignaldiscovered/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    echoLog('    - Software: ' + __json['header']['softwareName'] + ' / ' + __json['header']['softwareVersion'])
                    echoLog('        - ' + ('AUTHORISED' if __authorised else 'UNAUTHORISED'))

                    if __authorised:
                        if 'StarSystem' in __json['message']:
                            echoLog('    - System Name: ' + __json['message']['StarSystem'])
                        if 'SignalName' in __json['message']:
                            echoLog('    - Signal Name: ' + __json['message']['SignalName'])

                    echoLog('')

                # Handle Docking Granted events (station data)
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/dockinggranted/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                ...
                # Handle Approach Settlement events
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/approachsettlement/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        ...
                        if 'Name' in __json['message']:
                            echoLog('    - Settlement Name: ' + __json['message']['Name'])
                ...
                # Handle FSS Body Signals events
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/fssbodysignals/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        ...
                        if 'Signals' in __json['message']:
                            echoLog('    - Signals found: ' + str(len(__json['message']['Signals'])))
                ...
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/scanbarycentre/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                ...
                # Handle Codex Entry events
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/codexentry/1' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        if 'StarSystem' in __json['message']:
                            echoLog('    - System Name: ' + __json['message']['StarSystem'])
                        if 'Name' in __json['message']:
                            echoLog('    - Discovery Name: ' + __json['message']['Name'])
                ...
                # Handle Commodity v3 events (market data)
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/commodity/3' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        ...
                        if 'commodities' in __json['message']:
                            echoLog('    - Commodities: ' + str(len(__json['message']['commodities'])) + ' items')

                    echoLog('')

                # Handle Outfitting events (ship modules/equipment)
                elif __json['$schemaRef'] == 'https://eddn.edcd.io/schemas/outfitting/2' + ('/test' if (__debugEDDN == True) else ''):
                    echoLogJSON(__message)
                    ...
                    if __authorised:
                        ...
                        if 'modules' in __json['message']:
                            echoLog('    - Modules available: ' + str(len(__json['message']['modules'])))
